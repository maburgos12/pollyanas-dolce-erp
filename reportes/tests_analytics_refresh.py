...
class AnalyticsProductionFactWasteMappingTests(TestCase):
    ...
    def test_rebuild_production_facts_classifies_supply_waste_without_finished_good_fact(self):
        target_date = date(2026, 4, 2)
        branch = Sucursal.objects.create(codigo="CEDIS-T", nombre="CEDIS Test", activa=False)
        point_branch = PointBranch.objects.create(external_id="CEDIS", name="CEDIS", erp_branch=branch)
        insumo = Insumo.objects.create(nombre="Pan Vainilla Dawn Arándano - Chico")
        InsumoAlias.objects.create(nombre="Pan Arándano Chico", insumo=insumo)

        PointWasteLine.objects.create(
            branch=point_branch,
            movement_external_id="WASTE-APRIL-SUPPLY-1",
            source_hash="waste-april-supply-1",
            movement_at=timezone.make_aware(datetime(2026, 4, 2, 8, 0)),
            item_name="Pan Arándano Chico",
            quantity=Decimal("1.000"),
            total_cost=Decimal("17.63"),
        )

        inserted = rebuild_production_facts(start_date=target_date, end_date=target_date)

        self.assertEqual(inserted, 0)
        waste_line = PointWasteLine.objects.get(source_hash="waste-april-supply-1")
        self.assertIsNone(waste_line.receta_id)
        self.assertEqual(waste_line.insumo_id, insumo.id)
        self.assertFalse(FactProduccionDiaria.objects.filter(fecha=target_date).exists())
