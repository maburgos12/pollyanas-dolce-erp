...
class PointCostIntegrityAuditCommandTests(TestCase):
    ...
    def test_audit_point_cost_integrity_reports_impact_without_mutating_data(self):
        ...
        self.assertEqual(payload["summary"]["bad_cost_rows_total"], 1)
        self.assertEqual(payload["summary"]["inventory_alert_rows_total"], 2)
        self.assertEqual(payload["summary"]["monthly_bad_count"], 0)
        self.assertEqual(payload["summary"]["recipe_month_impacts_count"], 0)
        self.assertEqual(payload["summary"]["latest_unsafe_count"], 0)
        self.assertEqual(payload["monthly_bad"], [])
        self.assertEqual(payload["recipe_month_impacts_top"], [])
        self.assertEqual(payload["summary"]["point_code_collision_count"], 1)
        self.assertEqual(payload["summary"]["point_corrections_count"], 1)
