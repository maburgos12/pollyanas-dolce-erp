...
def closed_month_comparison(*, cutoff: date, previous_totals: dict | None = None) -> dict:
    """Validate current coverage, accepting total-network history for the same period."""
    ...
    bulk = get_daily_sales_bulk(fechas=read_days, sucursales=sorted(expected),
    ...
    if previous_totals is None:
        ...
        prev_amount = previous_totals.get('amount')
        prev_quantity = previous_totals.get('quantity')
        prev_missing = []
        previous_complete = prev_amount is not None and prev_quantity is not None
    ...
    elif pct is None and not gaps and prev_amount == 0:
    ...
    return {
        ...
        'comparison_label': f'{pct:+.1f}%' if pct is not None else 'N/D',
