...
class RecetasCosteoApiTests(TestCase):
    ...
    def test_endpoint_inventario_aliases_pendientes_export_csv_xlsx(self):
        PointPendingMatch.objects.create(
            tipo=PointPendingMatch.TIPO_INSUMO,
            point_codigo="PT-EXP-01",
            point_nombre="Harina export",
            method="FUZZY",
            fuzzy_score=90.0,
            fuzzy_sugerencia="Harina API",
        )
        receta_pending = Receta.objects.create(
            nombre="Receta pending export",
            sheet_name="Insumos pending export",
            hash_contenido="hash-api-pending-export-001",
        )
        LineaReceta.objects.create(
            receta=receta_pending,
            posicion=1,
            insumo=None,
            insumo_texto="Harina export",
            cantidad=Decimal("1.0"),
            unidad=self.unidad,
            unidad_texto="kg",
            match_status=LineaReceta.STATUS_NEEDS_REVIEW,
        ...
        AlmacenSyncRun.objects.create(
            ...
            pending_preview=[
                {
                    "nombre_origen": "Harina export",
                    "nombre_normalizado": "harina export",
                    "suggestion": "Harina API",
                    "score": 90,
                    "method": "FUZZY",
                    "fuente": "ALMACEN",
        ...
        url = reverse("api_inventario_aliases_pendientes")
        resp_csv = self.client.get(url, {"export": "csv", "limit": 50})
        self.assertEqual(resp_csv.status_code, 200)
        self.assertIn("text/csv", resp_csv["Content-Type"])
        self.assertIn("inventario_aliases_pendientes.csv", resp_csv["Content-Disposition"])
        csv_body = resp_csv.content.decode("utf-8")
        self.assertIn("seccion,id,tipo,codigo,nombre", csv_body)
        self.assertIn("Harina export", csv_body)

        resp_xlsx = self.client.get(url, {"export": "xlsx", "limit": 50})
        self.assertEqual(resp_xlsx.status_code, 200)
        self.assertIn(
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            resp_xlsx["Content-Type"],
        )
        self.assertIn("inventario_aliases_pendientes.xlsx", resp_xlsx["Content-Disposition"])
        self.assertTrue(resp_xlsx.content.startswith(b"PK"))

    def test_endpoint_inventario_aliases_pendientes_export_invalido(self):
        url = reverse("api_inventario_aliases_pendientes")
    ...
    def test_endpoint_integraciones_point_resumen(self):
        ...
        self.assertIn("errors_delta_pct", payload["api_24h_comparativo"])
        self.assertIn("requests_delta_pct", payload["api_24h_comparativo"])
        self.assertEqual(len(payload["api_7d"]["daily"]), 7)
        self.assertGreaterEqual(payload["api_7d"]["requests"], 1)
        self.assertIn("error_rate_pct", payload["api_7d"])
    ...
    def test_endpoint_compras_solicitudes_list_filters_and_totals(self):
        ...
        self.assertEqual(Decimal(payload["totales"]["presupuesto_estimado_total_filtered"]), Decimal("45.00"))
    ...
    def test_endpoint_compras_solicitudes_list_offset_and_totales_filtrados(self):
        proveedor = Proveedor.objects.create(nombre="Proveedor lista solicitudes offset", activo=True)
        CostoInsumo.objects.create(
            insumo=self.insumo,
            proveedor=proveedor,
            costo_unitario=Decimal("10.00"),
            source_hash="api-list-sol-off-1",
        )
        SolicitudCompra.objects.create(
            area="Compras",
            solicitante="uno",
        ...
        url = reverse("api_compras_solicitudes")
        resp = self.client.get(url, {"mes": "2026-02", "limit": 1, "offset": 1})
        self.assertEqual(resp.status_code, 200)
        payload = resp.json()
        self.assertEqual(payload["filters"]["offset"], 1)
        ...
        self.assertEqual(payload["totales"]["rows_returned"], 1)
        self.assertEqual(payload["totales"]["rows"], 1)
        self.assertEqual(payload["totales"]["by_status"][SolicitudCompra.STATUS_BORRADOR], 2)
        self.assertEqual(payload["totales"]["by_status"][SolicitudCompra.STATUS_APROBADA], 1)
    ...
    def test_endpoint_compras_ordenes_list_filters_and_totals(self):
        ...
        self.assertEqual(Decimal(payload["totales"]["monto_estimado_total_filtered"]), Decimal("120.50"))
    ...
    def test_endpoint_compras_ordenes_list_offset_and_totales_filtrados(self):
        ...
        solicitud_c = SolicitudCompra.objects.create(
            area="Compras",
            solicitante="api",
            insumo=self.insumo,
            proveedor_sugerido=proveedor,
        ...
        OrdenCompra.objects.create(
            ...
            proveedor=proveedor,
            fecha_emision=date(2026, 2, 14),
            monto_estimado=Decimal("80.00"),
            estatus=OrdenCompra.STATUS_BORRADOR,
        )
        OrdenCompra.objects.create(
            solicitud=solicitud_c,
            proveedor=proveedor,
            fecha_emision=date(2026, 2, 15),
        ...
        self.assertEqual(payload["totales"]["rows_returned"], 1)
        self.assertEqual(payload["totales"]["rows"], 1)
        self.assertEqual(payload["totales"]["by_status"][OrdenCompra.STATUS_BORRADOR], 2)
        self.assertEqual(payload["totales"]["by_status"][OrdenCompra.STATUS_ENVIADA], 1)
    ...
    def test_endpoint_compras_recepciones_list_offset_and_totales_filtrados(self):
        proveedor = Proveedor.objects.create(nombre="Proveedor recepciones offset", activo=True)
        solicitud = SolicitudCompra.objects.create(
            area="Compras",
            solicitante="api",
            insumo=self.insumo,
    ...
    def test_endpoint_ventas_pronostico_import_preview_y_confirm(self):
        preview_url = reverse("api_ventas_pronostico_import_preview")
        confirm_url = reverse("api_ventas_pronostico_import_confirm")
        payload = {
            "dry_run": False,
            "rows": [
                {
    ...
    def test_endpoint_ventas_pronostico_import_preview_confirm_requires_perm(self):
        user_model = get_user_model()
        user = user_model.objects.create_user(
            username="sin_perm_pronostico_import_api",
            email="sin_perm_pronostico_import_api@example.com",
            password="test12345",
        )
        self.client.force_login(user)

        payload = {"rows": [{"receta_id": self.receta.id, "periodo": "2026-05", "cantidad": "7"}]}
        for name in ("api_ventas_pronostico_import_preview", "api_ventas_pronostico_import_confirm"):
    ...
    def test_endpoint_ventas_solicitud_aplicar_forecast(self):
        ...
        self.assertGreaterEqual(payload["updated"]["applied"], 1)
    ...
    def test_endpoint_ventas_solicitud_aplicar_forecast_dry_run_no_persist(self):
        sucursal = Sucursal.objects.create(codigo="OESTE", nombre="Sucursal Oeste", activa=True)
        ...
        self.assertEqual(resp.status_code, 200)
        payload = resp.json()
        self.assertTrue(payload["updated"]["dry_run"])
