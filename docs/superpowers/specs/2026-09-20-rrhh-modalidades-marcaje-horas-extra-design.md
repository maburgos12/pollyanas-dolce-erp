...
PATCH parcial no puede recomponer silenciosamente un destino con campos de dos lecturas distintas: si empleado/fecha cambiaron desde la lectura que compuso el destino, responde 409 y exige recargar sin escribir. El guardado de `HoraExtra` abre una transacción que abarca `pre_save`, SQL y `post_save`, conservando `using`, `force_insert`, `force_update` y `update_fields`. Antes del SQL, `pre_save` adquiere las jornadas observada/destino/vinculada y relee la identidad bloqueada; si cambió mientras esperaba, aborta con `ValidationError`. Para `update_fields`, concilia la jornada realmente persistida, no atributos viejos que no se guardaron. Las adquisiciones de `post_save` son reentrantes. Admin y ORM directo comparten este contrato; `pre_delete` adquiere el conjunto de jornadas del lote antes de eliminar, también para borrado múltiple de admin. Fixtures `raw` no concilian ni consultan relaciones incompletas. `QuerySet.update`, `bulk_create` y SQL directo omiten las señales de Django: no son entradas operativas admitidas para resolver/corregir extras sin bloqueo explícito.
