...
- resolver POST en `services_horas_extra_autorizacion.resolver_hora_extra` dentro de `transaction.atomic()`: leer identidad sin bloqueo; adquirir advisory PostgreSQL estable namespaced por empleado/fecha para origen, destino y asistencia vinculada, ordenado por clave; después bloquear asistencias por pk y extras por pk, incluso en manuales sin asistencia. El helper `services_extra_bloqueos` es compartido con generador y ediciones. Adquirir jornada antes de filas/incidencias también en evaluación y conciliación; una transacción llamadora debe hacerlo antes de sus propios bloqueos. La misma clave es reentrante dentro de la transacción;
...
Proteger por `pre_delete` el borrado ORM (individual, queryset y cascadas) de asistencia vinculada; devolver 409 en API y conservar origen. No se modifica el esquema ni se promete protección ante SQL directo. Las ediciones genéricas y bonos deben releer bajo el mismo bloqueo antes de guardar, preservando campos históricos concurrentes. Validar correspondencia del vínculo para autorizar, pero permitir rechazo pendiente si una corrección de fecha dejó vínculo inconsistente; el generador no debe recrearlo ni trasladarlo. Agregar regresiones DELETE sin orfandad, fecha corregida→rechazo, PATCH obsoleto→auditoría intacta y primera asistencia confirmada tras SELECT vacío→generador esperando advisory sin deadlock, verificando `pg_blocking_pids`.

Llamadores transaccionales: `crear_ajuste_asistencia`/`aprobar_ajuste_asistencia` y `PointAttendanceSyncService.persist_payload` adquieren jornada antes de crear/bloquear/guardar asistencia; cubrir orden en las pruebas de ajustes y sync Point. La ingesta Hik separa proyección y efectos, y los importadores Hik/Excel guardan antes de entrar al generador, sin retener filas de asistencia en una transacción exterior. El barrido usa la entrada protegida `evaluar_dia_empleado`; las señales usan `conciliar_dia_extra`. Ninguna regla aritmética cambia en estos llamadores.

Cerrar entradas admin/ORM: `HoraExtra.save` mantiene atomic durante pre_save→SQL→post_save, sin cambios de esquema y preservando las opciones del ORM. Pre_save bloquea las jornadas observada, destino y vinculada antes de SQL; relee/revalida identidad y aborta ante desplazamiento concurrente. Derivar destino efectivo considerando `update_fields` y pasarlo a post_save evita adquirir una jornada nueva a partir de atributos viejos. Pre_delete cubre eliminación individual y lote admin en orden estable. Mantener raw fixtures sin conciliación. Probar admin create/edit/delete/multiple, autocommit, flags de save y deadlock real admin frente a resolver. Probar además PATCH que lee A/D1, otro request mueve A/D2 y luego intenta cambiar solo empleado B: debe responder409 sin escribir B/D2. Operaciones bulk que omiten señales/SQL directo requieren el contrato explícito y no sustituyen estos servicios.

Conservar rollback exterior con `HoraExtra.save` usando `savepoint=False`; probar overflow SQL real y ausencia de autorización/monto contradictorios. `pre_delete` de asistencia debe adquirir el conjunto completo del queryset/cascada antes de filas, no ordenar jornadas por PK; probar concurrencia real entre borrado admin y traslado HE con hashes invertidos. En cascadas empleado, incluir jornadas de ambos modelos antes de cualquier bloqueo. Si la identidad cambia dentro de `bloquear_hora_extra`, emitir conflicto distinguible y409 en APIs (toast/ancla en web), reservando404 para inexistentes; probar movimiento mientras se espera el advisory en RRHH y ambos bonos.
