...
| RRHH / Horas extra | Autorizar y rechazar desde la bandeja | Sí, toast global, bloqueo del botón y validación transaccional del saldo vigente | Sí, `#hora-extra-<id>`; error conserva el registro visible | RRHHViewsTests + prueba concurrente PostgreSQL + navegador local pendiente | Automatizado: automáticas sin turno/saldo vigente no se autorizan; manuales conservadas |
...
| Reportes / Expediente IMSS | Previsualizar una SUA con EMA/EBA opcional y aplicar el mismo expediente | Sí, `data-async-action`, toast global y bloqueo exclusivo del botón pulsado; la previsualización async conserva los archivos seleccionados | La previsualización no escribe; el éxito redirige al detalle estable y los errores conservan la selección para reintento | `reportes.tests_cedula_expediente.PantallaExpedienteTests` + navegador local | Automatizado; descarga privada y NSS enmascarado; producción pendiente |
