...
def es_dia_laborable(fecha: date) -> bool:
    if fecha.weekday() == 6:
        return False
    return (fecha.month, fecha.day) not in DESCANSOS_OFICIALES_FIJOS
...
def saldo_vacaciones_empleado(empleado: Empleado, *, periodo_anio: int | None = None) -> dict:
    periodo_anio = periodo_anio or timezone.localdate().year
    generado = dias_generados_para_empleado(empleado)
...
def crear_solicitud_vacaciones(*, empleado: Empleado, fecha_inicio: date, fecha_fin: date, motivo: str, actor=None) -> SolicitudVacaciones:
    ...
    saldo = saldo_vacaciones_empleado(empleado, periodo_anio=fecha_inicio.year)
