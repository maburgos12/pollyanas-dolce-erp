...
CORTES_BLOQUEADOS_AJUSTES = (
    PrenominaCorte.ESTADO_EXPORTADO,
    PrenominaCorte.ESTADO_CERRADO,
)


def _can_view_prenomina(user) -> bool:
    return can_view_submodule(user, "rrhh", "nomina")


def _can_manage_prenomina(user) -> bool:
    return can_manage_submodule(user, "rrhh", "nomina")
...
def _blocked_export_reasons(corte: PrenominaCorte) -> list[str]:
    ...
    if corte.estado in (PrenominaCorte.ESTADO_EXPORTADO, PrenominaCorte.ESTADO_CERRADO):
        reasons.append(f"corte {corte.get_estado_display().lower()}")
    checks = [
    ...
    reasons.extend(label for key, label in checks if _contador_resumen(resumen, key) > 0)
...
def _bloquear_ajuste_si_corte_cerrado(request, corte: PrenominaCorte) -> bool:
    if corte.estado not in CORTES_BLOQUEADOS_AJUSTES:
        return False
    messages.error(
        request,
        f"No se pueden modificar ajustes de asistencia porque el corte {corte.folio} ya está "
        f"{corte.get_estado_display().lower()}.",
    )
    return True


@transaction.atomic
def _marcar_corte_exportado(corte: PrenominaCorte) -> None:
    exportados = corte.movimientos.filter(estado=PrenominaMovimiento.ESTADO_LISTO).update(
        estado=PrenominaMovimiento.ESTADO_EXPORTADO,
        actualizado_en=timezone.now(),
    )
    resumen = dict(corte.resumen or {})
    resumen["movimientos_listos"] = 0
    resumen["movimientos_exportados"] = _contador_resumen(resumen, "movimientos_exportados") + exportados
    corte.estado = PrenominaCorte.ESTADO_EXPORTADO
    corte.resumen = resumen
    corte.save(update_fields=["estado", "resumen", "actualizado_en"])


@login_required
def prenomina(request):
    if not _can_view_prenomina(request.user):
        raise PermissionDenied("No tienes permisos para ver prenómina.")

    hoy = timezone.localdate()
    if request.method == "POST":
        if not _can_manage_prenomina(request.user):
    ...
    return render(
        ...
        "rrhh/prenomina.html",
        {
            ...
            "can_manage_rrhh": _can_manage_prenomina(request.user),
            "hoy": hoy.isoformat(),
...
def prenomina_ajuste_crear(request, pk, empleado_id):
    ...
    if request.method != "POST":
        return redirect_url
    if _bloquear_ajuste_si_corte_cerrado(request, corte):
        return redirect_url
    ...
    if not fecha or fecha < corte.fecha_inicio or fecha > corte.fecha_fin:
        ...
        return redirect_url
    if resumen.empleado.fecha_ingreso and fecha < resumen.empleado.fecha_ingreso:
        messages.error(request, "No se pueden capturar ajustes antes de la fecha de ingreso del empleado.")
        return redirect_url
...
def prenomina_export_contpaqi(request, pk):
    ...
    if reasons:
        ...
        return redirect("rrhh:prenomina_detail", pk=corte.pk)
    response = export_movimientos_contpaqi_xlsx(corte)
    _marcar_corte_exportado(corte)
    return response
