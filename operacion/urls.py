from django.urls import include, path

from . import views

app_name = "operacion"

urlpatterns = [
    path("conteos/", include("inventario.urls_conteos", namespace="conteos_app")),
    path("", views.app_home, name="app_home"),
    path("sucursal/", views.sucursal_tools, name="sucursal_tools"),
    path("sw.js", views.app_sw, name="app_sw"),
    path("activos/q/<uuid:qr_token>/", views.activo_pasaporte, name="activo_pasaporte"),
    path("activos/escanear/", views.activo_escanear, name="activo_escanear"),
    path("activos/buscar/", views.activo_buscar, name="activo_buscar"),
    path("api/fallas/activos/", views.fallas_activos_api, name="fallas_activos_api"),
    path("api/fallas/crear/", views.fallas_crear_api, name="fallas_crear_api"),
    path("higiene/", views.higiene_home, name="higiene_home"),
    path("higiene/guardar/", views.higiene_guardar, name="higiene_guardar"),
    path("higiene/historial/", views.higiene_historial, name="higiene_historial"),
    path("higiene/historial/imprimir/", views.higiene_imprimir, name="higiene_imprimir"),
    path("api/mermas-insumos/catalogo/", views.mermas_insumos_catalogo_api, name="mermas_insumos_catalogo_api"),
    path("api/mermas-insumos/crear/", views.mermas_insumos_crear_api, name="mermas_insumos_crear_api"),
    path(
        "api/mermas-insumos/<int:merma_id>/aprobar/",
        views.mermas_insumos_aprobar_api,
        name="mermas_insumos_aprobar_api",
    ),
    path(
        "api/mermas-insumos/<int:merma_id>/decidir/",
        views.mermas_insumos_decidir_api,
        name="mermas_insumos_decidir_api",
    ),
    path(
        "api/mermas-insumos/<int:merma_id>/reenviar/",
        views.mermas_insumos_reenviar_api,
        name="mermas_insumos_reenviar_api",
    ),
    path(
        "api/mermas-insumos/<int:merma_id>/reasignar/",
        views.mermas_insumos_reasignar_api,
        name="mermas_insumos_reasignar_api",
    ),
    path("bitacoras/", views.bitacoras_home, name="bitacoras_home"),
    path("bitacoras/apertura/", views.bitacoras_apertura, name="bitacoras_apertura"),
    path("bitacoras/<str:tipo>/", views.bitacora_captura, name="bitacora_captura"),
]
