(() => {
  ...
  const isPrintView = document.body.classList.contains('rrhh-print-page');
  const compactSections = document.querySelectorAll('[data-mobile-collapse="true"]');
  if (compactSections.length && !isPrintView) {
  ...
  if (isPrintView) document.querySelectorAll('.rrhh-permit-details').forEach(el => { el.open = true; });
})();
