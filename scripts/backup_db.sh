...
(
    ...
    if command -v sha256sum >/dev/null 2>&1; then
        sha256sum "$PREFIX.sql.gz" "$PREFIX.conteos.tar.gz"
    else
        shasum -a 256 "$PREFIX.sql.gz" "$PREFIX.conteos.tar.gz"
    fi
) > "$STAGING/$PREFIX.manifest"
...
# Only complete new sets participate. Legacy SQL dumps and incomplete sets stay
# untouched during transition; their removal requires a separate decision.
shopt -s nullglob
COMPLETE=()
for manifest in "$BACKUP_DIR"/backup_*.manifest; do
    base=${manifest##*/}
    base=${base%.manifest}
    if [[ "$base" =~ ^backup_[0-9]{8}_[0-9]{6}$ ]] &&
        [ -f "$BACKUP_DIR/$base.sql.gz" ] && [ -f "$BACKUP_DIR/$base.conteos.tar.gz" ]; then
        COMPLETE+=("$base")
    fi
done
...
for ((i=0; i<DELETE_COUNT; i++)); do
    old=${COMPLETE[$i]}
    rm -f "$BACKUP_DIR/$old.manifest" "$BACKUP_DIR/$old.sql.gz" "$BACKUP_DIR/$old.conteos.tar.gz"
