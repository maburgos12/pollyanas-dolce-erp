...
- API: POST /api/ventas/pronostico-estadistico/ (forecast por sucursal con banda inferior/superior + comparativo contra solicitud de ventas con estatus en rango y filtro opcional `min_confianza_pct`)
...
- API: POST /api/ventas/solicitud/aplicar-forecast/ (ajuste automático al forecast con `dry_run`, tope opcional `max_variacion_pct` y filtro opcional `min_confianza_pct`)
