...
- API: GET /api/ventas/pipeline/resumen/ (resumen ejecutivo + detalle por receta y por sucursal: historial vs pronóstico vs solicitud; filtros `status`/`delta_min`, límite `top_sucursales` y soporte `?export=csv|xlsx`)
