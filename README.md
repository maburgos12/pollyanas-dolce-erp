...
- API: GET /api/ventas/solicitud/list/ (solicitudes de ventas filtrables por alcance/sucursal/periodo; opcional `include_forecast_ref=1` para comparar contra pronóstico, filtros `forecast_status`/`forecast_delta_min` y soporte `?export=csv|xlsx`)
