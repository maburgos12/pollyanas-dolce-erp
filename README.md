...
- API: GET /api/integraciones/point/operaciones/historial/ (bitácora operativa de integraciones con filtros `action`, `date_from`, `date_to`, paginación `limit`/`offset`, ordenamiento `sort_by`/`sort_dir` y `export=csv|xlsx`)
