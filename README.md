...
- API: POST /api/inventario/aliases/pendientes-unificados/resolver/ (auto-resuelve alias cross-fuente con dry_run/commit, filtro `source`, ventana `limit`/`offset`, orden `sort_by`/`sort_dir` y totales `*_preview`)
