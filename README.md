...
- API: POST /api/ventas/pronostico-backtest/ (mide precisión histórica: MAPE/Bias por ventana, con escenario `base/bajo/alto` y `min_confianza_pct`)
...
- API: POST /api/ventas/pronostico-estadistico/ (forecast por sucursal con banda inferior/superior + comparativo contra solicitud en escenario `base/bajo/alto` y filtro opcional `min_confianza_pct`)
