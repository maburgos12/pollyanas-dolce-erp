...
class HallmarkGuardrailsStaticTests(SimpleTestCase):
    def test_base_template_loads_guardrails_after_module_css(self):
        ...
        self.assertIn("20260703-erp-group-tabs-v2", html)
    ...
    def test_base_template_includes_mobile_touch_navigation_shell(self):
        ...
        self.assertIn("20260903-report-point-v1", html)
        ...
        self.assertIn('{% if item.badge_count %}<span class="tab-badge">{{ item.badge_count }}</span>{% endif %}', html)
    ...
    def test_base_template_makes_erp_installable_as_pwa(self):
        ...
        self.assertIn("navigator.serviceWorker.register('/erp-sw.js?v=20260724-superuser-preview-v23')", html)
    ...
    def test_erp_pwa_manifest_and_service_worker_are_minimal(self):
        ...
        self.assertIn('const CACHE_NAME = "pollyanas-erp-shell-v27-recetas-costos-recuperacion"', sw)
        ...
        self.assertIn(b"pollyanas-erp-shell-v27-recetas-costos-recuperacion", b"".join(response.streaming_content))
    ...
    def test_guardrails_keep_erp_group_tabs_out_of_grid_layout(self):
        css = (Path(settings.BASE_DIR) / "static" / "css" / "hallmark_guardrails.css").read_text()
        self.assertIn(".erp-group-tabs", css)
        self.assertIn("grid-template-columns: none !important;", css)
