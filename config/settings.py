...
GOOGLE_ROADS_TIMEOUT_SECONDS = env_int("GOOGLE_ROADS_TIMEOUT_SECONDS", 10)
...
ECOMMERCE_API_TIMEOUT_SECONDS = env_int("ECOMMERCE_API_TIMEOUT_SECONDS", 10)
...
AGENTE_DG_WEBHOOK_SECRET = (os.getenv("AGENTE_DG_WEBHOOK_SECRET") or "").strip()
...
CONSOLIDADO_CEDIS_EXPORT_CC = [
    email.strip()
    for email in os.getenv("CONSOLIDADO_CEDIS_EXPORT_CC", "maburgos12@pollyanasdolce.com").split(",")
    if email.strip()
]
...
POINT_DELIVERY_SYNC_ENABLED = env_bool("POINT_DELIVERY_SYNC_ENABLED", default=False)
POINT_DELIVERY_SYNC_LOOKBACK_DAYS = max(
    7,
    min(env_int("POINT_DELIVERY_SYNC_LOOKBACK_DAYS", 7), 31),
)
POINT_DELIVERY_API_CLIENT_PREFIX = os.getenv(
    "POINT_DELIVERY_API_CLIENT_PREFIX",
    "",
).strip()
POINT_DELIVERY_API_CLIENT_ID = os.getenv("POINT_DELIVERY_API_CLIENT_ID", "").strip()
POINT_DELIVERY_SYNC_STALE_SECONDS = max(
    120,
    env_int("POINT_DELIVERY_SYNC_STALE_SECONDS", 300),
)
